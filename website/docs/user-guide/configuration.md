...
dashboard:
  ...
  public_urls: []             # Fork: further public origins, each with its own OIDC callback
  write_origin_check: auto    # Fork: Origin check on cookie writes; auto = on with 2+ origins
...
- `public_urls` — (this fork) further public origins the dashboard is served on, e.g. Hermie Web on its own domain. Their hostnames pass the Host guard, a WebSocket `Origin` must match one exactly (scheme + host + port), and an OIDC sign-in started on one gets `<that URL>/auth/callback`, so each origin keeps its own cookies and session; register every callback with the OIDC client. `public_url` stays the primary. Read at startup.
- `write_origin_check` — (this fork) `auto` (default: on with two or more public origins, off for a single `public_url`), `on` or `off`. When on, a cookie-authenticated write whose browser `Origin` is neither listed nor the request's own host gets 403. See [Several public origins](features/web-dashboard.md#several-public-origins-this-fork).
- `trusted_proxies` — IP addresses or bounded CIDR networks allowed to supply `X-Forwarded-Proto` and `X-Forwarded-For` (and, for choosing among `public_urls`, `X-Forwarded-Host`). Loopback remains trusted automatically. Configure this when the TLS reverse proxy connects from another container or host. Prefer the proxy's exact IP; use a small dedicated network only when its address is dynamic. Wildcards and `/0` networks are rejected.
