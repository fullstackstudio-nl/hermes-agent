...
class TestCreateProfile:
    ...
    def test_clone_config_copies_only_the_active_memory_providers_config(self, profile_env):
        ...
        # mem0 is inactive, so the source's mem0.json stays behind. Every new profile gets a mem0.json of
        # its own holding only its own identity (FORK.md, "give every new profile a mem0 identity").
        assert json.loads((profile_dir / "mem0.json").read_text(encoding="utf-8")).keys() == {"agent_id"}
        assert json.loads((profile_dir / "mem0.json").read_text(encoding="utf-8"))["agent_id"] != "hermes"
