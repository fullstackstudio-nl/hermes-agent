...
| `feat(tui_gateway): write the author of a turn onto the row it persists` | A user row written for a submitted turn carries `display_metadata.author` = `{"id": "<provider>:<user id>", "name"?}`, the identity of the connection that submitted it, and `gateway.capabilities` advertises `per_message_author` so a client knows whether this gateway attributes messages at all before it draws anything. Nothing is written for a turn nobody submitted, or for a transport that names no login. A turn's identity decides whom it works for, never who wrote its row: a row is authored only from the identity a caller holds beside that exact text, so the `/goal` continuation and every other turn the gateway starts itself still run scoped as the person whose work they continue and name no author. A steer or redirect carries its own sender: the gateway hands the signed-in sender to the agent with the text (`steer(text, author=)`, `redirect(text, author=)`), and the agent keeps the two together in the pending slot, under that slot's lock, through every drain and put-back. The steer or redirect row it lands as mid-turn names that sender, and one handed back after the final answer (`pending_steer_author`) is queued under them. Text more than one person, or a connection naming no login, contributed to names nobody -- never the owner of the turn it arrived in. Two user rows joined for role alternation (the pre-call repair, a micro-compaction supersede, a task restated onto the compaction handoff, the per-call copies) keep an author only when both rows had that same author, and a row a compaction summary was folded into names nobody. A stored row's words run again are WRITTEN by that row's author and RUN as whoever asked: `/retry` runs the retried turn on the gateway itself and answers with an `exec` line instead of handing the text back for the presser's client to resubmit; the row names the rewound row's author (or nobody) and, when the presser is somebody else, `display_metadata.replayed_by` = the presser's `{"id", "name"?}`, so a client can show "Robin (retried by Sam)" -- additive, a client that does not read it is unaffected. The turn's scope (memory, tools, approvals, `HERMES_SESSION_USER_*`) is the presser's: somebody having sent words once is not consent to run them again at another time on somebody else's action. A retry that finds the session busy queues with the same split. A truncating `prompt.submit` (the desktop's regenerate and edit) follows the same rule when it resends the replaced row's own words; new words over the sender's own row are theirs, and new words over anybody else's row name nobody. `/undo` and `session.undo` refuse a row that names somebody other than the presser ("You can only undo your own last message in a shared chat."), because the undone words land in the presser's composer one Send from being theirs; a row that names nobody undoes as before. A queued prompt drained into an isolated turn is authored on the gateway from its envelope, not by the child from the turn's identity. | A client cannot tell who wrote a message it did not send. `message.start` is contracted as an empty payload and two clients on one session share a `FanoutTransport`, so the socket says nothing about the other person's turn; the row has no author column and no wire field, so a reload leaves nothing to read either. Every client therefore treats an unmarked user row as its own, and a colleague's sentence is painted as the reader's -- a false statement, not a gap, and one no client can repair for itself. A turn the gateway replays or starts itself is held to the same rule: the first version took a requeued steer's author from the turn it arrived in, which stored a colleague's steer as the turn owner's words, and authored the `/goal` continuation as the person whose goal it was, who never typed it. The steer's sender lives in the agent rather than in a gateway-side record because only the slot's own lock makes text and sender change together: a record beside it has to be read at the turn's end, after the session is released, where a second turn can finish and a later steer can land first. A wrong name is stored for good; a missing one only costs a label. The commit above resolves who submitted a turn but keeps it in process; this writes that one fact down where it survives, in the free-form `display_metadata` dict every row already has and both the REST read and the WS history projection already forward whole. It fails closed the same way as the two commits above: an absent author is a state every reader handles, and nothing is written rather than something guessed. | Only if upstream gives a message an author of its own. The right shape is a `messages.author_id` column, and that is a migration a fork should not carry -- so the key is named `author` rather than anything fork-specific, and the whole change is upstreamable as it stands. Offer it with #118439 once the two identity commits above land. |
...
| `fix(memory): give every new profile a mem0 identity of its own` | Every path that makes a profile -- `create_profile()` (so `hermes profile create` with or without `--clone`/`--clone-all`, `POST /api/profiles` and the `profiles.create` RPC), `hermes profile import` and a distribution install -- writes an `agent_id` into the new profile's own `mem0.json`: `hermes-<name>-<random>`. It does so for every new profile, whether or not it uses mem0 today, so a profile that turns mem0 on later is already isolated; the file is inert until then. A `--clone-all` copy keeps the source's `search_agent_ids` and other mem0 settings, with the entry that meant the source's own scope rewritten to the clone's; a plain `--clone` never copied `mem0.json` and still does not, so it gets the identity and nothing else. A distribution reinstall or update keeps the identity the profile has, from `mem0.json` or `.env`. A named profile that starts without any identity -- one made before this -- keeps the `hermes` it was running under, but the provider writes it into that profile's `mem0.json` with `"agent_id_source": "legacy-default"` and logs a warning naming the profile and the file on every start until an operator decides. That write only adds to a file the gateway can read, parse as an object and owns, keeps its mode, and re-reads it in the same step, so an identity saved meanwhile is the one that runs; a malformed, unreadable or foreign file is left byte-for-byte alone with a warning, and a malformed file copied into a new profile is kept beside it as `mem0.json.invalid`. A `MEM0_AGENT_ID` set in the profile's own `.env` after the pin replaces it -- one in the process environment does not -- and the pin is removed, with the `hermes` entry of an explicit `search_agent_ids` moved to the new identity. `hermes memory setup` and the dashboard's memory form always offer the identity the profile would run under now: its own files', else a `MEM0_AGENT_ID` from the service environment when that profile is the one the process runs as (never for a profile the dashboard edits by parameter, whose scope falls through to the launch profile's environment), else -- for a named profile whose config loads and names another memory provider -- a new one of its own. A named profile on mem0 without an identity, or whose config is missing or cannot be read, is offered `hermes`, and saving that writes the pin marker, because every save writes the form's `agent_id` and one that only changes another setting must not switch its memory. `hermes` typed where an identity of its own was offered is a choice and is not marked; the wizard writes `mem0.json` before it switches the provider, so that is judged on the config as it was. Saving an identity of its own over the fallback -- pinned or not yet -- drops the marker and moves the `hermes` entry of an explicit `search_agent_ids` the same way. The non-interactive OSS setup no longer overwrites the identity a profile has. | The mem0 provider falls back to `agent_id` `hermes` when a profile names none, and `hermes` is also what the default profile runs under -- the fallback and the setup wizard's default. A new profile had no `mem0.json`, and a clone or the RPC's credential mirror copied the source's `.env`, `MEM0_AGENT_ID` included, so a bot made from the app or the CLI read and wrote the default profile's memories without anyone having chosen that. On a gateway several people use that is one bot reading another's memory. Past rows cannot be separated afterwards: a row carries no profile marker, only its `agent_id`. The suffix keeps a profile made later under a deleted or renamed profile's name from inheriting its memories. Existing profiles are pinned rather than switched because a switch drops everything they recall today out of recall, and an explicit `search_agent_ids` that lists `hermes` as the profile's own would fail closed and leave it with no memory at all. | Only if upstream gives profiles a memory identity of their own. Offer it upstream with the carried isolation; nothing in it is fork-specific. |
...
- **`display.busy_input_mode` (default `interrupt`) redirects a message typed mid-turn INTO the
  ...
  `interrupt` while the turn is live. The redirect or steer row it lands as does name its sender, so
  the transcript is right about who said it; what runs is still A's turn. A steer that arrives too
  late for the turn is handed back and runs as its own turn under the person who sent it.

## Where the author touches agent core

The per-message author is the one change here that edits agent core, so an upstream merge can conflict
with it. The functions to look at first, the hottest first:

- `agent/interrupt_control.py`: `InterruptControlMixin.redirect` (upstream keeps reworking it),
  `.steer`, `.interrupt`, `.clear_interrupt`, `_drain_pending_steer` / `_drain_pending_redirect` and
  their `_entry` twins, and the module helpers beside the mixin.
- `agent/conversation_loop.py`: `_apply_active_turn_redirect`.
- `agent/turn_iteration_prep.py`: `prepare_iteration`, `begin_iteration`, `apply_retry_restarts`,
  `_inject_steer_after_newest_tool_result`, and the helpers they now call (`inject_pending_steer`,
  `apply_pending_redirect`, `requeue_unapplied_redirect`).
- `agent/turn_finalizer.py`: the leftover block in `finalize_turn` (`hand_back_leftover_steer`).
- `agent/agent_runtime_helpers.py`: `apply_pending_steer_to_tool_results`, `_requeue_pending_steer`,
  `_merge_consecutive_users`, `drop_thinking_only_and_merge_users`.
- `agent/prompt_builder.py`: `steer_user_row`. `agent/agent_init.py`: `_CONTROL_STATE`.
- The other user-row joins: `MicroCompactionMixin._merge_adjacent_user_turns`,
  `ContextCompressor._reappend_inflight_user_task`, `ContextCompressor._merge_summary_into_tail_row`,
  `moa_alternation.merge_same_role_messages`.
- Replays: `hermes_state_rewind.RewindOutcome` / `rewind_user_turn` (the rewound row's `author`),
  `tui_gateway/methods_tools._cmd_retry`, and the truncation block of `prompt.submit`
  (`_truncate_history_for_submit`).

It fails closed. A drain site upstream adds that still calls the plain `_drain_pending_*` only drops the
author, and a row that names nobody is the safe state; a slot rewritten any other way no longer matches
the text its sender was recorded against, so it names nobody too. A rename or a signature change fails
loudly in the tests rather than quietly. The one thing a merge can get wrong silently is a new place
that joins two user rows without `keep_shared_author` -- grep for new merges of adjacent user rows.
...
## Profiles that ran without a memory identity

After the update, every named profile that uses mem0 and never had an `agent_id` of its own keeps
running under `hermes` -- the same memories as before -- and says so. Its first start writes
`"agent_id": "hermes"` and `"agent_id_source": "legacy-default"` into that profile's `mem0.json`, and
every start logs a warning that names the profile and the file. Find them with:

```
grep -l '"agent_id_source": "legacy-default"' ~/.hermes/profiles/*/mem0.json
```

A profile that has not started since the update has no `mem0.json` yet, so it is not in that list:
any named profile with `memory.provider: mem0` in its `config.yaml` and neither an `agent_id` in its
`mem0.json` nor `MEM0_AGENT_ID` in its `.env` is one too. So is a profile made before the update that
never used mem0 and turns it on later: nothing tells it apart from one that shared all along, so it is
pinned to `hermes` too, and the warning is what says so. A profile whose `mem0.json` cannot be read or
parsed, or belongs to another user, is not pinned at all: it runs under `hermes` and logs that the file
was left untouched. Fix the file and it is pinned on the next start. A gateway that runs as a
different user than the one owning the profiles' files -- root over files owned by a service user,
say -- therefore never pins an existing `mem0.json`; it warns on every start instead. Give the files
to the gateway's user, or set the identity yourself.

For each one, decide:

- **It should have its own memory** (a bot made for somebody else, a customer's bot): put
  `MEM0_AGENT_ID=<new name>` in the profile's own `.env` -- the warning suggests a name -- or save a new
  agent identifier in the dashboard's memory form. Either replaces the pin on the next start, removes
  it, and moves a `hermes` entry in an explicit `search_agent_ids` to the new name. Editing
  `mem0.json` by hand works too: set `"agent_id"`, delete `"agent_id_source"`, and change `"hermes"`
  in `"search_agent_ids"` to the new name as well, or the profile fails closed with no memory at all
  (that list must contain the profile's own `agent_id`). A `MEM0_AGENT_ID` in the service
  environment does not un-pin: it is not the profile's own setting. The profile starts empty. What it stored before stays under `hermes`, where the default profile can still
  recall it; mem0 rows carry no profile marker, so the two cannot be told apart and moved.
- **Sharing is intended**: delete `"agent_id_source"` and leave `"agent_id": "hermes"`. The warning
  stops. Sharing a team layer rather than the default's own memory is `search_agent_ids`.

Check also any profile whose `.env` names the same `MEM0_AGENT_ID` as the default profile's: a clone or
the app's credential mirror copied that line into it, and it has shared the default's memory since. A
line in `.env` counts as configured, so such a profile is neither pinned nor warned
about. Profiles made after the update are not affected: their own `mem0.json` names their own identity
and outranks the copied line.

Restart the gateway (or open a new session) after editing; the identity is read when the agent is built.
